...
def contactPopulation(Aclass, RAD_nAtoms, atom_resname, atom_resid, waterTuple, weight):
    ...
    if RAD_nAtoms is not None:
        ...
        for nInfo in RAD_nAtoms:
            ...
            nResname = nInfo[2]
            if (
                nResname not in waterTuple
                and atom_resname not in waterTuple
                and nResid != atom_resid
            ):
                if (nResname, nResid) not in Aclass.resid_contact_matrix_dict[
                    (atom_resname, atom_resid)
                ]:
                    Aclass.resid_contact_matrix_dict[(atom_resname, atom_resid)][
                        (nResname, nResid)
                    ] = 0
                    Aclass.resid_contact_matrix_dict[(nResname, nResid)][
                        (atom_resname, atom_resid)
                    ] = 0
                Aclass.resid_contact_matrix_dict[(atom_resname, atom_resid)][
                    (nResname, nResid)
...
def contactPopulationUA(
    Aclass, RAD_nAtoms, atom_name, atom_resname, atom_resid, waterTuple, weight
):
    """ """
    ...
    if RAD_nAtoms is not None:
        ...
        for nInfo in RAD_nAtoms:
            ...
            if (
                ...
                if (nAtom, nResname, nResid) not in Aclass.resid_contact_matrix_dict[
                    (atom_name, atom_resname, atom_resid)
                ]:
                    Aclass.resid_contact_matrix_dict[
                        (atom_name, atom_resname, atom_resid)
                    ][(nAtom, nResname, nResid)] = 0
                    Aclass.resid_contact_matrix_dict[(nAtom, nResname, nResid)][
                        (atom_name, atom_resname, atom_resid)
                    ] = 0
                Aclass.resid_contact_matrix_dict[(atom_name, atom_resname, atom_resid)][
...
def SorPopulation(
    ...
    for RR in [[RADshell_num_dist, str(RADshell_num)], ["0", "0"]]:
        ...
        for ADs in [["A", As], ["D", Ds]]:
            if (
                ADs[1]
                not in Aclass.RADshell_Sor_dict[nearest_resname][
                    (atom_resname, N_H, atom_name)
                ][(RADshell_num_dist2, str(RADshell_num2))][RAD_str][ADs[0]]
            ):

                Aclass.RADshell_Sor_dict[nearest_resname][
                    (atom_resname, N_H, atom_name)
                ][(RADshell_num_dist2, str(RADshell_num2))][RAD_str][ADs[0]][ADs[1]] = 0

            Aclass.RADshell_Sor_dict[nearest_resname][(atom_resname, N_H, atom_name)][
    ...
    try:
        if (
            ...
            try:
                ...
                # (w_stored + w)
                forces_add = (forces_stored * weight_stored + force * weight) / float(
                    weight_stored + weight
                )
                torques_add = (
                ...
                if KE is not None:
                    try:
                        ...
                        if DOF == 0:
                            T = KE * 240.663461
                        T_add = (T + T_stored * count_stored) / float(count_add)
...
def SvibPopulation(
    ...
    if "0" not in Aclass.RADshell_dict[nearest_resname][atom_info]:
        Aclass.RADshell_dict[nearest_resname][atom_info]["0"] = [0, 0, 0, 0, 0, 0, 0]

    for RADshell_num_dist2 in [RADshell_num_dist, "0"]:
        forces_stored = Aclass.RADshell_dict[nearest_resname][atom_info][
            RADshell_num_dist2
        ][0]
        torques_stored = Aclass.RADshell_dict[nearest_resname][atom_info][
            RADshell_num_dist2
        ][1]
        PE_stored = Aclass.RADshell_dict[nearest_resname][atom_info][
            RADshell_num_dist2
        ][2]
        KE_stored = Aclass.RADshell_dict[nearest_resname][atom_info][
            RADshell_num_dist2
        ][3]
        T_stored = Aclass.RADshell_dict[nearest_resname][atom_info][RADshell_num_dist2][
            4
        ]
        count_stored = Aclass.RADshell_dict[nearest_resname][atom_info][
            RADshell_num_dist2
        ][5]
        weight_stored = Aclass.RADshell_dict[nearest_resname][atom_info][
            RADshell_num_dist2
        ][6]
        ...
        if count_add != count_stored and PE is not None:
            ...
            PE_add = (PE_stored * weight_stored + PE * weight) / float(
                weight_stored + weight
            )
            KE_add = (KE_stored * weight_stored + KE * weight) / float(
                weight_stored + weight
            )

        Aclass.RADshell_dict[nearest_resname][atom_info][RADshell_num_dist2][
            0
        ] = forces_add
        Aclass.RADshell_dict[nearest_resname][atom_info][RADshell_num_dist2][
            1
        ] = torques_add
        Aclass.RADshell_dict[nearest_resname][atom_info][RADshell_num_dist2][2] = PE_add
        Aclass.RADshell_dict[nearest_resname][atom_info][RADshell_num_dist2][3] = KE_add
        Aclass.RADshell_dict[nearest_resname][atom_info][RADshell_num_dist2][4] = T_add
        Aclass.RADshell_dict[nearest_resname][atom_info][RADshell_num_dist2][
            5
        ] = count_add
        Aclass.RADshell_dict[nearest_resname][atom_info][RADshell_num_dist2][
    ...
    if type(MweightedForces) is not type(None):  # and \
        ...
        if RADshell_num_dist not in Aclass.WM_FT_shell_dict[nearest_resname][atom_info]:
            Aclass.WM_FT_shell_dict[nearest_resname][atom_info][RADshell_num_dist] = [
                0,
                0,
                0,
                0,
        ...
        forces_stored = Aclass.WM_FT_shell_dict[nearest_resname][atom_info][
            RADshell_num_dist
        ][0]
        torques_stored = Aclass.WM_FT_shell_dict[nearest_resname][atom_info][
            RADshell_num_dist
        ][1]
        PE_stored = Aclass.WM_FT_shell_dict[nearest_resname][atom_info][
            RADshell_num_dist
        ][2]
        KE_stored = Aclass.WM_FT_shell_dict[nearest_resname][atom_info][
            RADshell_num_dist
        ][3]
        T_stored = Aclass.WM_FT_shell_dict[nearest_resname][atom_info][
            RADshell_num_dist
        ][4]
        UAforces_stored = Aclass.WM_FT_shell_dict[nearest_resname][atom_info][
            RADshell_num_dist
        ][5]
        UAtorques_stored = Aclass.WM_FT_shell_dict[nearest_resname][atom_info][
            RADshell_num_dist
        ][6]
        count_stored = Aclass.WM_FT_shell_dict[nearest_resname][atom_info][
            RADshell_num_dist
        ][7]
        weight_stored = Aclass.WM_FT_shell_dict[nearest_resname][atom_info][
            RADshell_num_dist
        ][8]
    ...
    if nearest_resname is not None and dihedral_phi_list is not None:
        ...
        for dihedral_phi_type in dihedral_phi_list:
            ...
            for b in all_bins:
                if (
                    b
                    not in Aclass.adaptive_dih_dict[nearest_resname][assigned][
                        RADshell_num_dist
                    ][dihedral_atoms]
                ):
                    Aclass.adaptive_dih_dict[nearest_resname][assigned][
                        RADshell_num_dist
                    ][dihedral_atoms][b] = 0
            Aclass.adaptive_dih_dict[nearest_resname][assigned][RADshell_num_dist][
                dihedral_atoms
            ][
                rounded_phi
            ] += weight  # 1
